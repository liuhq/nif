...
function UISelect:init(items, opts, on_done)
    ...
    local max_width = popup_options.relative == 'editor' and vim.o.columns - 4 or
        vim.api.nvim_win_get_width(0) - 4
    ...
    for index, item in ipairs(items) do
        --[[
                        -- the codes is got from nui wiki, but it's going to have a bug here when using with mini.sessions
                        -- mini.sessions passes the format_item arg which only handle string array and not table
                    ...
                    --]]
        local item_text = string.sub(format_item(item), 0, max_width)
        menu_items[index] = Menu.item(item_text, { __raw_item = item, index = index })
    end
